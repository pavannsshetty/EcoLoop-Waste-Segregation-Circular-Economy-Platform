        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {tasks.map(task => (
                        <div key={task._id} className={`p-5 rounded-lg border flex flex-col md:flex-row md:items-center gap-4 ${dk('bg-white/5 border-gray-800', 'bg-white border-slate-100 shadow-sm')}`}>
                            ...
                            <div className="shrink-0 flex gap-2 sm:self-center self-end">
                                {task.status === 'Pending' && (
                                    <button onClick={() => handleUpdateTask(task._id, 'In Progress')} className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-xs font-bold shadow-sm hover:bg-indigo-500 transition">
                                        Start Task
                                    </button>
                                )}
                                {task.status === 'In Progress' && (
                                    <button onClick={() => handleUpdateTask(task._id, 'Completed')} className="px-4 py-2 rounded-lg bg-green-500 text-white text-xs font-bold shadow-sm hover:bg-green-500 transition flex items-center gap-2">
