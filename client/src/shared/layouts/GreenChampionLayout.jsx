...
const NAV_MAIN = [
  { path: '/green-champion/dashboard', Icon: () => <HiSparkles className="h-5 w-5" />, label: 'Dashboard' },
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className={`text-[10px] font-semibold uppercase tracking-wide ${sectionLbl} hidden sm:inline`}>Champion</span>
          </div>
          <button
            type="button"
        ...
        <header className={`h-16 backdrop-blur-sm border-b sticky top-0 z-30 flex items-center px-4 sm:px-6 gap-4 shadow-sm ${headerBg}`}>
          ...
          <div className="flex-1 min-w-0">
            <p className={`text-sm font-semibold truncate ${dark ? 'text-slate-200' : 'text-slate-800'}`}>
              {greeting}, {user.name || 'Green Champion'}
            </p>
            <p className={`text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Green Champion</p>
          </div>
          <button
            type="button"
            onClick={toggleDark}
          ...
          <div className="h-9 w-9 rounded-full overflow-hidden bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-sm">
