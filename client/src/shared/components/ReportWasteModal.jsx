...
const SuccessModal = ({ reportId, onClose, dark }) => (
  <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
    ...
    <div className={`relative z-10 w-full max-w-sm rounded-none shadow-2xl p-8 text-center space-y-6 ${dark ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
      ...
      <div>
        <h3 className={`text-xl font-bold ${dark ? 'text-white' : 'text-slate-900'}`}>Report Submitted!</h3>
        <p className={`text-sm mt-2 font-medium ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
          Our team is alerted. You can track this report in real-time.
        </p>
      </div>
...
const ReportWasteModal = ({ isOpen, onClose, onSuccess, dark = false }) => {
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      // Wait for user to see success modal
    } catch (err) { 
        ...
        <div className={`relative z-10 w-full max-w-md sm:max-w-2xl rounded-none sm:rounded-none shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] pointer-events-auto ${dark ? 'bg-black/90 border border-gray-800' : 'bg-white'}`}>
          ...
          <form onSubmit={handleSubmit} noValidate className="overflow-y-auto flex-1 px-4 sm:px-6 py-4 space-y-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pointer-events-auto">
            ...
            <div className={card}>
              ...
              <div>
                ...
                <Dropdown value={form.severity} onChange={e => set('severity', e.target.value)}
                  placeholder="Select Severity Level" className="mt-1">
              ...
              <div>
                <label className={lbl}>Description <span className={`text-xs ml-1 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>(Optional)</span></label>
                <textarea rows={2} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  placeholder="Example: Garbage dumped near bus stop beside temple road." className={`${inp} mt-1 resize-none`} />
              </div>
            </div>
            ...
            <div className={card}>
              ...
              {locMethod === 'map' && <MapPicker onLocationSelect={handleLocationSelect} villageName={user?.village} dark={dark} />}
              {locMethod === 'manual' && (
