...
const ConfirmationModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-md rounded-sm shadow-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in duration-300 ${dark ? 'bg-[#151B23] border border-white/5' : 'bg-white'}`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div className={`h-12 w-12 rounded-sm ${theme.bg} dark:bg-opacity-20 flex items-center justify-center`}>
        ...
        <div className={`px-6 py-4 flex flex-col sm:flex-row gap-3 ${dark ? 'bg-white/5' : 'bg-slate-50'}`}>
          <button onClick={onClose}
            className="flex-1 px-4 py-2.5 rounded-sm text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
            {cancelText}
          </button>
          <button onClick={() => { onConfirm(); onClose(); }}
            className={`flex-1 px-4 py-2.5 rounded-sm text-sm font-bold text-white shadow-lg shadow-red-500/20 transition-all active:scale-95 ${theme.btn}`}>
            {confirmText}
          </button>
        </div>
      </div>
