...
import { HiRefresh, HiGlobe, HiChevronDown, HiChevronUp, HiCheckCircle, HiX, HiPencil, HiTruck, HiFlag } from 'react-icons/hi';
...
const STATUS = {
  ...
  Verified:      { border: 'border-l-green-500' },
  ...
  'Clarification Requested': { border: 'border-l-purple-500' },
  Resubmitted:   { border: 'border-l-indigo-500' },
  'Clarification Expired': { border: 'border-l-gray-500' },
};
...
const PublicWasteReports = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      setReports(all.filter(r => !r.reportType || r.reportType === 'Public'));
  ...
  useEffect(() => {
    ...
    const handler = (updated) => {
      ...
      setReports((rs) => {
        ...
        if (!updated.reportType || updated.reportType === 'Public') filtered.push(updated);
        return filtered;
      });
    };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 animate-in fade-in duration-500 overflow-hidden">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className={`text-lg font-bold tracking-tight ${dk('text-slate-200', 'text-slate-800')}`}>Public Waste Reports</h1>
          <p className={`text-sm font-medium mt-0.5 ${dk('text-slate-400', 'text-slate-500')}`}>Your community waste reports</p>
        </div>
      ...
      {!loading && !error && reports.length === 0 && (
        <div className={`flex flex-col items-center justify-center py-20 gap-3 text-center ${dk('text-slate-500', 'text-slate-400')}`}>
          <HiGlobe className="h-12 w-12 opacity-30" />
          <p className="text-sm">No public waste reports yet.</p>
              ...
              {open && (
                <>
                  ...
                  <div className="px-4 sm:px-5 py-3 space-y-2">
                    ...
                    {r.wasteSeenAt && (
                      <div className={`text-xs ${dk('text-slate-500', 'text-slate-400')}`}>
                        <span className={`font-semibold ${dk('text-slate-300', 'text-slate-600')}`}>Waste seen:</span> {r.wasteSeenAt}
                      </div>
                    )}
                    {r.landmarkType && (
                      <div className={`text-xs ${dk('text-slate-500', 'text-slate-400')}`}>
                        <span className={`font-semibold ${dk('text-slate-300', 'text-slate-600')}`}>Near:</span> {r.landmarkType}{r.landmark ? ` — ${r.landmark}` : ''}
                      </div>
                    )}
                    {r.completionNotes && (
                      <div className={`text-xs ${dk('text-slate-500', 'text-slate-400')}`}>
                    ...
                    {r.status === 'Clarification Requested' && r.clarificationRequests?.length > 0 && (
                      <div className={`rounded-lg border p-3 space-y-2 ${dk('bg-purple-900/10 border-purple-500/30', 'bg-purple-50 border-purple-200')}`}>
                        <span className={`text-xs font-semibold ${dk('text-purple-300', 'text-purple-700')}`}>Clarification Requested</span>
                        {r.clarificationRequests.map((cr, i) => (
                          <div key={i} className={`text-xs ${dk('text-slate-300', 'text-slate-600')}`}>
                            <span className="font-semibold">Reason {i + 1}:</span> {cr.reason || 'N/A'}
                            {cr.requestedAt && <span className="block opacity-70">Requested: {fmt(cr.requestedAt)}</span>}
                          </div>
                        ))}
                        {r.clarificationExpiresAt && (
                          <span className="block text-[10px] italic opacity-60">Expires: {fmt(r.clarificationExpiresAt)}</span>
                        )}
                      </div>
                    )}
                    {r.supportedBy?.length > 0 && (
                      <div className={`text-xs ${dk('text-slate-400', 'text-slate-500')}`}>
                        <span className={`font-semibold ${dk('text-slate-300', 'text-slate-600')}`}>Supported by:</span> {r.supportedBy.length} {r.supportedBy.length === 1 ? 'person' : 'people'}
                      </div>
                    )}
                    {r.duplicateOf && (
                      <div className={`text-xs ${dk('text-orange-400', 'text-orange-600')}`}>
                        <span className="font-semibold">Flagged as duplicate</span>
                      </div>
                    )}
                  </div>
                </>
