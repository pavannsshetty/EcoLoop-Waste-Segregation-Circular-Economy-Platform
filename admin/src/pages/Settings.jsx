...
const Settings = () => {
  ...
  const card = dk('bg-white/5 rounded-2xl border border-gray-700 p-6 space-y-6', 'bg-white rounded-2xl border border-slate-100 p-6 space-y-6 shadow-sm');
  ...
  const inp = dk(
    'w-full max-w-md rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2.5 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition',
    'w-full max-w-md rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition shadow-sm'
  );
  ...
  return (
    <div className="page-container max-w-4xl space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="page-header">Portal Settings</h1>
        <p className="page-subheading">Manage your administrative preferences and system automation</p>
      </div>

      {success && (
        <div className="flex items-center gap-2 rounded-xl bg-green-500 border border-green-500 px-4 py-3 text-sm text-green-500">
          <HiCheckCircle className="h-5 w-5 shrink-0" /> {success}
        </div>
      )}

      <form onSubmit={handleSave} className="space-y-6">
        <div className={`rounded-3xl border shadow-sm p-6 space-y-6 ${dk('bg-white/5 border-gray-700', 'bg-white border-slate-100')}`}>
        ...
        <div className="flex justify-end gap-3 pt-2">
           <button type="button" onClick={() => window.location.reload()} className={`px-5 py-2.5 rounded-xl border text-sm font-medium transition ${dk('border-slate-700 text-slate-300 hover:bg-slate-800', 'border-slate-200 text-slate-600 hover:bg-slate-50')}`}>
             Cancel
           </button>
           <button type="submit" disabled={loading} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-500 text-white text-sm font-semibold hover:bg-green-500 transition disabled:opacity-60">
