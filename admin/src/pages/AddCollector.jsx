...
const Field = ({ label, required, labelClass, children }) => (
  <div>
...
const AddCollector = () => {
  ...
  const sectionTitle = dk('text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3', 'text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3');
  const btnOutline = dk(
    'px-3 rounded-xl border border-slate-700 text-slate-400 hover:text-green-400 hover:border-green-600 transition',
    'px-3 rounded-xl border border-slate-200 text-slate-500 hover:text-green-600 hover:border-green-400 transition'
  );
  const btnGhost = dk(
    'flex-1 rounded-xl border border-slate-700 text-slate-300 py-2.5 text-sm font-semibold hover:bg-slate-800 transition',
    'flex-1 rounded-xl border border-slate-200 text-slate-700 py-2.5 text-sm font-semibold hover:bg-slate-50 transition'
  );
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-5">
      ...
      {success && (
        <div className="flex items-center gap-2 rounded-xl bg-green-100 border border-green-200 dark:bg-green-900/30 dark:border-green-700 px-4 py-3 text-sm text-green-800 dark:text-green-400">
          <HiCheckCircle className="h-5 w-5 shrink-0" /> {success}
        </div>
      )}
      {errors.submit && (
        <div className="rounded-xl bg-red-100 border border-red-200 dark:bg-red-900/30 dark:border-red-700 px-4 py-3 text-sm text-red-800 dark:text-red-400">
          {errors.submit}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        <div className={card}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Collector / Team Name" required labelClass={labelClass}>
            ...
            <Field label="Mobile Number" required labelClass={labelClass}>
              <input
                type="tel"
                value={form.mobile}
        ...
        <div className={card}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="City" required labelClass={labelClass}>
            ...
            <Field label="Area / Locality" required labelClass={labelClass}>
            ...
            <Field label="Ward Number" required labelClass={labelClass}>
              <input type="text" value={form.ward} onChange={(e) => set('ward', e.target.value)} placeholder="e.g. Ward 12" className={inp} />
              {errors.ward && <p className="text-xs text-red-500 mt-0.5">{errors.ward}</p>}
            </Field>
          </div>
          <Field label="Team Size (1–5)" labelClass={labelClass}>
            <input
              type="number"
              min={1}
              max={5}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Collector ID (Auto)" labelClass={labelClass}>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={fetchNextId} className={btnOutline}>
                  <HiRefresh className="h-4 w-4" />
                </button>
              </div>
            </Field>
            <Field label="Password" required labelClass={labelClass}>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => set('password', genPassword())} className={`${btnOutline} text-xs font-medium whitespace-nowrap`}>
                  Auto
                </button>
              </div>
              ...
              <button
                ...
                className={`px-5 py-2 rounded-xl border text-sm font-semibold transition ${
                  form.status === s
                    ...
                    : dk('border-slate-700 text-slate-400 hover:border-slate-500', 'border-slate-200 text-slate-600 hover:border-slate-300')
                }`}
              >
                {s}
              </button>
