...
import { HiRefresh, HiEye, HiX, HiExclamationCircle, HiLocationMarker } from 'react-icons/hi';
...
const ModalWrapper = ({ children, onClose, title, dk }) => {
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex p-4 sm:p-6 bg-black/60 overflow-y-auto" onClick={(e) => { if(e.target === e.currentTarget) onClose(); }}>
      <div className={`relative m-auto w-full max-w-4xl max-h-full flex flex-col rounded-2xl border shadow-2xl ${dk('bg-slate-900 border-slate-700', 'bg-white border-slate-200')}`}>
        <div className={`px-4 sm:px-6 py-4 border-b flex justify-between items-center shrink-0 rounded-t-2xl sticky top-0 z-10 ${dk('border-slate-800 bg-slate-900', 'border-slate-100 bg-white')}`}>
    ...
    <div className="page-container space-y-8 animate-in fade-in duration-500">
      <div className="page-header-section">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <h1 className="page-header">Citizen Reports</h1>
            <p className="page-subheading">Track and manage all waste reports across the platform</p>
          </div>
          <button onClick={fetchReports} className="btn-refresh" title="Refresh reports">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {reports.map((r) => (
                  <tr key={r._id} className={`border-b transition ${dk('border-gray-800/50 hover:bg-white/5', 'border-slate-100 hover:bg-green-500/50')}`}>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Name:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.userId?.name || 'N/A'}</span></p>
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Phone:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.userId?.phone || 'N/A'}</span></p>
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Email:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.userId?.email || 'N/A'}</span></p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Type:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.wasteType}</span></p>
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Quantity:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.quantity || 'N/A'}</span></p>
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Severity:</span> <span className={`font-semibold ${getSeverityColor(viewModal.severity)}`}>{viewModal.severity}</span></p>
                  <p><span className={`inline-block w-24 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Created:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{new Date(viewModal.createdAt).toLocaleString()}</span></p>
                  <p className="flex items-start gap-1">
                    <span className={`inline-block w-24 shrink-0 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Village:</span> 
              ...
              <div>
                ...
                <div className="space-y-2 text-sm mb-4">
                  <p><span className={`inline-block w-32 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>House/Building:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.houseNo || '—'}</span></p>
                  <p><span className={`inline-block w-32 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Landmark:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.landmark || '—'}</span></p>
                  <p><span className={`inline-block w-32 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Seen:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.wasteSeenAt}</span></p>
                  <p><span className={`inline-block w-32 font-medium ${dk('text-slate-400', 'text-slate-500')}`}>Report Upvotes:</span> <span className={dk('text-slate-200', 'text-slate-800')}>{viewModal.upvotes?.length || 0}</span></p>
                  {viewModal.escalated && (
            ...
            <div className="space-y-6">
               <div>
                 ...
                 <div className="space-y-4 text-sm">
                   ...
                   <div className={`p-4 rounded-xl border ${dk('bg-slate-800 border-slate-700', 'bg-white border-slate-200 shadow-sm')}`}>
                     ...
                     {viewModal.assignedCollector ? (
                        <div>
                          ...
                          <p className={`text-xs mt-1 font-mono px-2 py-0.5 rounded inline-block ${dk('bg-green-500/40 text-green-500', 'bg-green-500/50 text-green-500')}`}>ID: {viewModal.assignedCollector.collectorId}</p>
                        </div>
                     ) : (
               ...
               <div>
                 ...
                 <div className={`p-4 rounded-xl border space-y-4 ${dk('bg-slate-800/50 border-slate-700', 'bg-slate-50 border-slate-200')}`}>
               ...
               <div>
                 ...
                 {viewModal.image ? (
                   ...
                   <div className={`h-40 rounded-xl flex items-center justify-center border border-dashed ${dk('border-slate-700 bg-slate-800', 'border-slate-300 bg-slate-50')}`}>
