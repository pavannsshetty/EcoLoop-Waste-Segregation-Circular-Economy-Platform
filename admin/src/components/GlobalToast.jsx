...
const TYPE_META = {
  ...
  champion:      { icon: '🌟', color: 'border-l-green-500', bg: 'bg-green-50' },
  approval:      { icon: '✅', color: 'border-l-purple-500', bg: 'bg-purple-50' },
  account:       { icon: '👤', color: 'border-l-red-500', bg: 'bg-red-50' },
  system:        { icon: '⚙️', color: 'border-l-slate-500', bg: 'bg-slate-50' },
  alert:         { icon: '🔴', color: 'border-l-rose-500', bg: 'bg-rose-50' },
  broadcast:     { icon: '📢', color: 'border-l-yellow-500', bg: 'bg-yellow-50' },
...
const getMeta = (type) => {
  ...
  if (lower.includes('report') || lower.includes('waste')) return TYPE_META.report;
  if (lower.includes('pickup') || lower.includes('home')) return TYPE_META.pickup;
  if (lower.includes('scrap')) return TYPE_META.scrap;
  if (lower.includes('order') || lower.includes('eco') || lower.includes('shopping')) return TYPE_META.order;
  if (lower.includes('champion')) return TYPE_META.champion;
  if (lower.includes('approval')) return TYPE_META.approval;
  if (lower.includes('account') || lower.includes('delet') || lower.includes('suspend')) return TYPE_META.account;
  if (lower.includes('alert') || lower.includes('emergency') || lower.includes('critical')) return TYPE_META.alert;
  if (lower.includes('broadcast') || lower.includes('announce')) return TYPE_META.broadcast;
  if (lower.includes('system')) return TYPE_META.system;
  return TYPE_META[type] || { icon: '📢', color: 'border-l-gray-500', bg: 'bg-gray-50' };
};

const TYPE_NAV = {
  report:   '/admin/reports/public',
  pickup:   '/admin/reports/home-pickup',
  scrap:    '/admin/reports',
  order:    '/admin/eco-shopping',
  champion: '/admin/champion-requests',
  approval: '/admin/approval-requests',
};
...
export default function GlobalToast() {
  ...
  const deriveNavPath = useCallback((notification) => {
    ...
    if (title.includes('report') || type.includes('report') || title.includes('waste')) return TYPE_NAV.report;
    if (title.includes('pickup') || type.includes('pickup')) return TYPE_NAV.pickup;
    if (title.includes('scrap') || type.includes('scrap')) return TYPE_NAV.scrap;
    if (title.includes('order') || type.includes('order') || title.includes('eco-shopp')) return TYPE_NAV.order;
    if (title.includes('champion') || type.includes('champion')) return TYPE_NAV.champion;
    if (title.includes('approval') || type.includes('approval')) return TYPE_NAV.approval;
